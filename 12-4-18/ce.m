...
findAllGroups := function(level)
  ...
  for divisor in divisors do
    ...
    for grp_ in subgroups do
      ...
      //weed out groups
      SL2Level := computeLevelSl2(grp meet SL(2, Integers(level)),level);
      GL2Level := computeLevelGl2(grp,level);

      if GL2Level ne level or SL2Level eq GL2Level then continue grp_; end if;
      ...
      if not isMissingTraceAndNew(grp,level) then continue grp_; end if;
      ...
      Write(saveFile,GL2Level);
      Write(saveFile,"*************************");
      count := count + 1;
    end for;
  end for;
  print "Done:",level;
  Write(saveFile,"function done going over groups of level:");
  Write(saveFile,level);
  return count;
