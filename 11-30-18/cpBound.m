...
findAllGroups := function(level)
  saveFile:= "11-30-18/cpBound.txt";
  SL2:=SL(2,Integers(level));
  divisors := Divisors(#SL2);
  count := 0;
  for divisor in divisors do
    subgroups := Subgroups(SL2:IndexEqual:=divisor);
    for grp_ in subgroups do
      ...
      Write(saveFile, "Structure of abelianization");
      Write(saveFile, Invariants(grp/CommutatorSubgroup(grp)));
      Write(saveFile,"*************************");
